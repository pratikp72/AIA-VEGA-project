...
/**
 * Captures raw request body for /api/feedback-submission/submit before the body parser.
 * Fixes empty ctx.request.body when requests come from the frontend (browser).
 * Stores parsed body in ctx.state.feedbackBody and replaces the consumed stream
 * so the body parser can still read it (avoids "stream is not readable" error).
 */
module.exports = (config, { strapi }) => {
  return async (ctx, next) => {
    ...
    if (ctx.method === 'POST' && isFeedbackSubmit) {
      try {
        const chunks = [];
        ...
        if (raw && raw.trim()) {
          ctx.state.feedbackBody = JSON.parse(raw);
        }
